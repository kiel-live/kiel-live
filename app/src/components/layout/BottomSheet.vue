<template>
  <div
    v-show="isOpen"
    class="absolute right-0 bottom-0 left-0 z-10 flex w-full flex-col rounded-t-3xl border-t border-gray-100 bg-white px-4 pt-2 pb-0 transition dark:border-neutral-800 dark:bg-neutral-950 dark:text-gray-300"
    :class="{
  ...
  >
    <button type="button" class="-mt-4 w-full touch-none pt-4 pb-4" :title="$t('drag_to_resize')" @pointerdown="drag">
      <div class="mx-auto h-1 w-12 shrink-0 rounded-full bg-gray-300 dark:bg-neutral-700" />
    </button>
    <slot />
  </div>
</template>
